...
function Server (torrent, opts) {
  ...
  function onRequest (req, res) {
    ...
    function handleRequest () {
      ...
      if (Number.isNaN(index) || index >= torrent.files.length + 1) {
        return serve404Page()
      }
    ...
    function serveIndexPage () {
      ...
      var listHtml = torrent.files.map(function (file, i) {
        return '<li><a download="' + file.name + '" href="/' + (i + 1) + '/' + file.name + '">' + file.path + '</a> ' +
    ...
    function serveArchive () {
      ...
      res.setHeader(
        'Content-Disposition',
        'inline; filename*=UTF-8\'\'' + encodeRFC5987(torrent.name + '.zip')
      )

      // Archive does not support range-requests
      res.setHeader('Accept-Ranges', 'none')

      var bytes = torrent.files.reduce(function (sum, file) {
        return sum + file.length + 92 + 2 * file.path.length
      }, 0)
      bytes += 22
      ...
      torrent.files.forEach((file) => {
        ...
        archive.append(rs, { name: file.path })
      })
